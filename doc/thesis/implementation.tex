...
Similarly, \myref{DSA} is used primarily for authentication.
...
\myref{AES} was also provided, however, using it (in \myref{Enc} and \myref{EncDet}) required tweaking its parameters, as per the protocol specification.
...
To begin with, \myref{DSA} key generation had to work, and, as mentioned, this was quite cumbersome.
...
Proper \myref{DSA} signatures on top of the respective messages and proof that the respective \myref{DSA} signature verification mechanism worked right needed to be accomplished.
