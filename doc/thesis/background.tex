...
On the other hand, Nigori was interesting from the point of view of protocol analysis, as well.
...
On a different note, a different focus for Nigori, as a protocol, is on the increasing level of diversity presented to application developers.
...
\myfig{Synchronization and separation of trust in Nigori. TODO: reference}{sync}{0.8}
...
The difference in determinism comes in the use of either a fixed (\myref{kiv}) or random initialization vector (IV) for \myref{AES}.
...
Another great benefit of using Mirage was that it, together with all of its supporting libraries, are writen primarily in OCaml.
...
One final great aspect of using Mirage as a development backend and OCaml as our language of choice is the ease of integration work.
...
Certain cryptographic building blocks were available using the \myref{cryptokit} library, thus contributing to the primitives required by Nigori.
For storage, a backend Object Relational Mapping (ORM) is also available, by using the \myref{ocaml-orm} library.
