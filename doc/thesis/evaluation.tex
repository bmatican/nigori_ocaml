...
The exact working of \myref{PBKDF} was left completety to its supporting RFC.
The description for the deterministic version of content encryption, \myref{EncDet}, was slightly misleading, in the sense that internally, it uses an \myref{HMAC} and then shirkns that to the fixed size of an \myref{AES} initialization vector, by splitting up the \myref{HMAC} into pieces of the requires size and applying xor among all of them, which was not directly apparent from the specification.
